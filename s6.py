...
def hero():
    ...
    Firstend = now.replace(hour=20, minute=6, second=0, microsecond=0)
    i=0
    while(y):
        now = datetime.datetime.now()
        if(now < First):
...
if(expirydate>today):
    ...
    rava=20220501391
    now = datetime.datetime.now()
    Second = now.replace(hour=17, minute=30, second=0, microsecond=0)
    Secondend = now.replace(hour=19, minute=55, second=0, microsecond=0)
